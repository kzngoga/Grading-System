...
if(isset($_GET['removeTchr'])){
    $tchrId = $_GET['removeTchr'];    
    $select = "Select * FROM teacher WHERE teacher_ID = '$tchrId'";

    $run_select = mysqli_query($con,$select);
    $fetch_select = mysqli_fetch_array($run_select);
    $password = $fetch_select['password'];
    $name = $fetch_select['teacher_Fname'];
    $lname = $fetch_select['teacher_Lname'];
}
?>
...
<?php
$remove = "update teacher set status = 'Off',password = '' where teacher_ID = '$tchrId'";
...
if($run_remove){
    ...
    Swal.fire({
      ...
      html:
      '<h1 class=alert-title>Success</h1>' + '<p class=alert-text>Teacher `$lname $name` was Successfully Removed from the system!</p>, ' + '<a href=../view_teachers.php class= alert-btn>Continue!</a>',
      type: 'success',
      animation: true,
      customClass: {
