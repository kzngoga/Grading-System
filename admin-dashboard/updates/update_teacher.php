...
if(isset($_GET['editTchr'])){
    $tchrId = $_GET['editTchr'];
    $select = "Select teacher.teacher_ID,teacher.teacher_Fname,
    ...
    where teacher.teacher_ID = '$tchrId'
    ORDER BY teacher_Lname";  
    $run_select = mysqli_query($con,$select);
    $fetch_select = mysqli_fetch_array($run_select);
    $tchrFname = $fetch_select['teacher_Fname'];
    $tchrLname = $fetch_select['teacher_Lname'];
    $email = $fetch_select['teacher_Mail'];
    $gender = $fetch_select['teacher_Gen'];
    $address = $fetch_select['teacher_Address'];
    $phone = $fetch_select['teacher_Phone'];
...
<html lang="en">
<head>
<title><?php echo $fname;?> | Update Teacher</title>
    ...
    <main role="main" class="col-md-10 ml-sm-auto px-4">
      <div class="dash-output-section">
        ...
        <form method="post" action="">
          <div class="form-row">
            ...
            <input class="form-control" type="text" Placeholder="Enter Lastname" 
                name="lname" id="lname" value="<?php echo $tchrLname;?>"readonly>
            </div>    
            <div class="form-group col-md-4">
            <label for="fname">Teacher's Firstname</label>
            <input class="form-control" type="text" Placeholder="Enter Lastname" 
                name="fname" id="fname" value="<?php echo $tchrFname;?>"readonly>
            </div>
            <div class="form-group col-md-4">
          ...
          <div class="form-row mt-3">
            ...
            <select class="custom-select" name="department" id="department" required>
                ...
                <optgroup label="Change Department">
                    <?php
                    $select_box = "select * From department";
        ...
        <div class="form-group">
            <button class="form-control btn btn-primary" id="addBtn" type="submit" name="update"> <span>Update Teacher</span></button>
...
$fetch_email_data = "Select teacher.teacher_Fname,
teacher.teacher_Mail,department.dep_Name FROM teacher INNER JOIN department 
ON teacher.dep_ID = department.dep_ID WHERE department.dep_ID = '$depart'";

$run_email_data = mysqli_query($con, $fetch_email_data);
$row_email_data = mysqli_fetch_array($run_email_data);
$email = $row_email_data ['teacher_Mail'];
$uname = $row_email_data ['teacher_Fname'];
...
Welcome To Grading System, you've been recently added to the system As a teacher in &nbsp;" .  " '$dep' Department" . ", <br> Your Login Details are:</p>";
...
teacher_Gen = '$gender',teacher_Phone ='$phone', status = '$stat', password = '$hashPass', 
dep_ID ='$depart' WHERE teacher_ID = '$tID'";
...
if($run_update){
    ...
    Swal.fire({
      ...
      html:
      '<h1 class=alert-title>Success</h1>' + '<p class=alert-text>Teacher `$Lname $Fname` was Successfully re-added to the system!</p>, ' + '<a href=../view_teachers.php class= alert-btn>Continue!</a>',
      type: 'success',
      animation: true,
      customClass: {
