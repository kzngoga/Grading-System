...
if(isset($_GET['course'])){
    $cour = $_GET['course'];
    $select = "select * from courses where course_ID = '$cour'";
...
<html lang="en">
<head>
<title><?php echo $fname;?> | View Marks</title>
  ...
  <div class="row">
    ...
    <main role="main" class="col-md-10 ml-sm-auto px-4">
      <div class="dash-output-section">
        <h2>Students Marks (<?php echo $cname;?>)</h2>
        <ul class="nav sub-nav">
           <li class="nav-item">
            <p class="nav-link sub-nav-head"><i class="fas fa-info-circle"></i> Class Info.</p>
          </li>     
          <li class="nav-item">
            <a href="view_marks_shift1.php?course=<?php echo $cour;?>" class="nav-link">
              <i class="fas fa-sun sub-icon"></i> 8h - 10h
            </a>
          </li>               
          <li class="nav-item">
            <a href="view_marks_shift2.php?course=<?php echo $cour;?>" class="nav-link">
              <i class="fas fa-sun sub-icon"></i> 11h - 13h
            </a>
          </li>           
          <li class="nav-item">
            <a href="view_marks_shift3.php?course=<?php echo $cour;?>" class="nav-link">
              <i class="fas fa-cloud-sun sub-icon"></i> 15h - 17h
            </a>
          </li>           
          <li class="nav-item">
            <a href="view_marks_shift4.php?course=<?php echo $cour;?>" class="nav-link active">
              <i class="fas fa-cloud-moon sub-icon"></i> 18h - 20h
            </a>
          </li>          
          <li class="nav-item">
            <a href="view_marks_shift5.php?course=<?php echo $cour;?>" class="nav-link">
              <i class="fab fa-weebly sub-icon"></i> Weekend
            </a>
          </li>          
        </ul>
        <div class="info-display">
          <h3 class="">Students List
          (Shift 18H - 20H)</h3>  
          <div class="table-responsive mt-4">
              ...
              <th>Edit</th>
               ...
               <?php
                $select = "Select student.student_Fname,student.student_Lname,
                        marks.marks_Test,marks.marks_Exam,marks.marks_ID,marks.verdict,
                        marks.total,department.dep_Name,courses.course_Name,
                        courses.total_Marks FROM student 
                        LEFT JOIN marks ON student.student_ID = marks.student_ID INNER JOIN 
                        department ON department.dep_ID = student.dep_ID LEFT JOIN courses 
                        ON courses.course_ID = marks.course_ID WHERE department.dep_ID = '$departID' 
                        AND courses.course_ID = '$cour' AND student.status = 'On' AND student.student_Shift = '18h-20h' 
                        ORDER BY student.student_Lname";
                $run_select = mysqli_query($con, $select);
                ...
                // echo $verdict;
                ?></td>
                <td><a href="updates/update_marks.php?marks=<?php echo $markId;?>">
            ...
            if($page>1){
              ...
              class="page-link" 
              href="view_marks_shift4.php?course='. $cour .'&page='. ($page-1) .'" style="background: #1ca48c; color: #f2f2f2;"> <i class="fas fa-caret-left"></i> Prev</a></li>';
            }             
            //display the links to the pages
            for($i=1; $i<=$nber_of_Pages; $i+=1){
                echo '<li class="page-item"><a
                class="page-link" 
                href="view_marks_shift4.php?course='. $cour .'&page='. $i .'"> '. $i .' </a></li>';
            }
            if($i-1>$page){
            echo '<li class="page-item"><a
            class="page-link" 
            href="view_marks_shift4.php?course='. $cour .'&page='. ($page+1) .'" style="background: #1ca48c; color: #f2f2f2;"> Next <i class="fas fa-caret-right"></i></a></li>';
            }
            ?>
            </ul> 
        </div>      
