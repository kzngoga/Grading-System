...
if(isset($_GET['editCrs'])){
    $courseId = $_GET['editCrs'];
    ...
    ON courses.dep_ID = department.dep_ID WHERE courses.course_ID = '$courseId'";
    $run_select = mysqli_query($con,$select);
    $fetch_select = mysqli_fetch_array($run_select);
    $cName = $fetch_select['course_Name'];
    $depName = $fetch_select['dep_Name'];
    $depId = $fetch_select['dep_ID'];
    $marks = $fetch_select['total_Marks'];
...
<html lang="en">
<head>
<title><?php echo $fname;?> | Update Courses</title>
  ...
  <div class="row">
    ...
    <main role="main" class="col-md-10 ml-sm-auto px-4">
      <div class="dash-output-section">
        <h2 class="text-center">Update Course In <?php echo $depName;?></h2>
        <center>
        <form method="post" action="" style="width: 30% !important;">
          <div class="form-group">
              <label for="cname" style="text-align: center !important;">Course Name</label>
              <input type="text" name="cname" class="form-control" id="cname" placeholder="Enter Course" required value="<?php echo $cName;?>">
          </div>
          <div class="form-group">
              ...
              value="<?php echo $depName;?>" readonly>
          </div>
          <div class="form-group">
              <label for="marks" style="text-align: center !important;">Overall Marks </label>
              <input type="text" name="marks" class="form-control" id="marks" placeholder="Enter Course Marks" value="<?php echo $marks;?>" required>
          </div>                         
        <div class="form-group">
            <button class="form-control btn btn-primary" id="update" type="submit" name="update" style="width: 70% !important;"> <span>Update Course</span></button>
...
$courName = $_POST['cname'];
$totMarks = $_POST['marks'];
$select = "select * from courses where course_Name = '$courName'";
$run_select = mysqli_query($con, $select);
$check_data = mysqli_num_rows($run_select);

if($Status == "Off") {
$update = "Update courses
set course_Name = '$courName', status = 'On' where course_ID = '$cID'";
...
if($run_update){
  ...
  Swal.fire({
    ...
    html:
    '<h1 class=alert-title>Success</h1>' + '<p class=alert-text>Course `$courName` was Successfully re-added</p>, ' + '<a href=../view_courses.php?department=$dID class= alert-btn>Continue!</a>',
    type: 'success',
    animation: true,
    customClass: {
...
if(($status=='On') && ($courName == $courseName)){
  echo "<script>window.open('../view_courses.php?department=$dID','_self')</script>";
}
...
elseif(($check_data > 0) && ($status=='On')){
  ...
  Swal.fire({
    ...
    allowOutsideClick:false,
    html:
    '<h1 class=alert-title>Error</h1>' + '<p class=alert-text>Course `$courName` Already added</p>, ' + '<a href=../view_courses.php?department=$dID class= alert-error-btn>Continue!</a>',
    type: 'error',
    animation: true,
    customClass: {
      popup: 'animated swing'
...
$update = "Update courses
set dep_ID = '$dID',course_Name = '$courName',total_Marks='$totMarks' 
WHERE course_ID = '$cID'";
...
if(($run_update) && ($status=='On')){
    ...
    Swal.fire({
      ...
      allowOutsideClick:false,
      html:
      '<h1 class=alert-title>Success</h1>' + '<p class=alert-text>Course `$courName` Successfully Updated</p>, ' + '<a href=../view_courses.php?department=$dID class= alert-btn>Continue!</a>',
      type: 'success',
      animation: true,
      customClass: {
        popup: 'animated swing'
