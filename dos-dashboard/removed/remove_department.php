...
<?php
// $remove = "Delete FROM department WHERE dep_ID = '$deptId'";
$remove = "Update department set status = 'Off' WHERE dep_ID = '$deptId'";
...
if($run_remove){
    ...
    Swal.fire({
      ...
      html:
      '<h1 class=alert-title>Success</h1>' + '<p class=alert-text>Department $name Is No Longer Available</p>, ' + '<a href=../view_department.php class= alert-btn>Continue!</a>',
      type: 'success',
      animation: true,
      customClass: {
    ...
    echo "<script>
    document.getElementById('error').innerHTML = 'Department not updated'</script>";
}
?>